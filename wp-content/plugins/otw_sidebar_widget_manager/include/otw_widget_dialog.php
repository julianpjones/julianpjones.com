...
//validat input data
if( !$sidebar || !$widget ){
	wp_die( __( 'Invalid sidebar or widget', 'otw_sbm' ) );
}
...
if( !isset( $wp_registered_sidebars[ $sidebar ] ) ){
	wp_die( __( 'Requested not registered sidebar', 'otw_sbm' ) );
}
...
if( !isset( $sidebar_widgets[ $sidebar ] ) || !count( $sidebar_widgets[ $sidebar ] ) || !in_array( $widget, $sidebar_widgets[ $sidebar ]  ) ){
	wp_die( __( 'Requested widget is not assinged to this sidebar', 'otw_sbm' ) );
}
...
<div class="d_info">
	<p><img src="<?php echo $otw_sbm_plugin_url.'/images/selected.gif';?>" alt=""/><?php _e('Means the widget will be displayed on that page, post, category, etc.', 'otw_sbm');?></p>
	<p><img src="<?php echo $otw_sbm_plugin_url.'/images/not_selected.gif';?>" alt=""/><?php _e('Means  the widget will be hidden form that page, post, category, etc', 'otw_sbm');?></p>
				...
				<div class="postbox">
					...
					<div class="inside sitems<?php if( count( $wp_item_data[0] ) > 15 ){ echo ' mto';}?>" id="otw_sbm_app_type_<?php echo $wp_item_type?>" rel="<?php echo $sidebar?>|<?php echo $widget?>|<?php echo $wp_item_type?>" >
						<div class="all_vis_lnks">
							<a href="javascript:;" rel="<?php echo $sidebar?>|<?php echo $widget?>|<?php echo $wp_item_type?>|vis" class="all_vis<?php echo otw_sidebar_item_all_class( 'vis', $sidebar, $widget, $wp_item_type )?>"><?php _e( 'all visible', 'otw_sbm' )?></a>
							<a href="javascript:;" rel="<?php echo $sidebar?>|<?php echo $widget?>|<?php echo $wp_item_type?>|invis"class="all_invis<?php echo otw_sidebar_item_all_class( 'invis', $sidebar, $widget, $wp_item_type )?>"><?php _e( 'all invisible', 'otw_sbm' )?></a>
							<?php if( !in_array( $wp_item_type, array( 'templatehierarchy', 'pagetemplate', 'archive' ) ) ){?>
									<input type="text" class="q_filter" value="<?php _e('Type to search', 'otw_sbm');?>"/>
									<div class="otw_app_loading"></div>
							<?php }?>
						</div>
						<?php if( count( $wp_item_data[0] ) > 15 ) {?>
							<div class="more_items"><?php echo sprintf( __( 'Showing 15 of %d items. Please use the filter box to filter them out', 'otw_sbm' ), count( $wp_item_data[0] ) );?></div>
						<?php }?>
						<div class="lf_items">
						</div>
